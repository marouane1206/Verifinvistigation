...
-- Drop existing problematic policies on journalist_approval_tokens
...
-- Profiles: Allow all authenticated users to read, only owner or admin can modify
...
-- Journalist Applications: Allow all authenticated users access to their own, admins can see all
...
-- Journalist Approval Tokens: Allow all authenticated users access
CREATE POLICY "tokens_select" ON public.journalist_approval_tokens FOR SELECT TO authenticated USING (true);
CREATE POLICY "tokens_insert" ON public.journalist_approval_tokens FOR INSERT TO authenticated WITH CHECK (true);
CREATE POLICY "tokens_update" ON public.journalist_approval_tokens FOR UPDATE TO authenticated USING (true);
DROP POLICY IF EXISTS "tokens_delete" ON public.journalist_approval_tokens;
CREATE POLICY "tokens_delete" ON public.journalist_approval_tokens FOR DELETE TO authenticated USING (true);
